...
class CarteController extends AbstractController
{
    ...
    public function addCard(Request $request) {
        if ($this->isGranted('ROLE_GCARTE')|| $this->isGranted('ROLE_PARTENAIRE')) {
            ...
            $userTab = $this->serializer->decode($user, 'json');
            foreach ($userTab as $key => $value) {
                $mat = 'SC'.$this->userController->random().date('Y').$this->userController->random1().substr($value['phone'], -3);
                ...
                $user = $this->serializer->denormalize($value, 'App\Entity\User');
                $user->setMat($mat)
